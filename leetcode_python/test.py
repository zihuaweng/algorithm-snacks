...
def word_get_handler(args):
    ...
    links = {"1" : ["post", "execution", None],
             ...
             "4" : ["get", "previous_word", [filename, word_index-1]]}
    return rep, links

def word_get_previous_handler(args):
    filename = args[0]; word_index = args[1]
    if word_index >= 0:
        word_info = data[filename][word_index]
    else:
        word_index = -1
        word_info = ("no previous words", 0)
    rep = '\n#{0}: {1} - {2}'.format(word_index+1, word_info[0], max(0, word_info[1]))
    rep += "\n\nWhat would you like to do next?"
    rep += "\n1 - Quit" + "\n2 - Upload file"
    rep += "\n3 - See next most-frequently occurring word"
    rep += "\n4 - See previous most-frequently occurring word"
...
handlers = {"post_execution" : quit_handler,
            ...
            "get_previous_word" : word_get_previous_handler}
